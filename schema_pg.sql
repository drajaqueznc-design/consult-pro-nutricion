...
-- ============================================================
CREATE TABLE IF NOT EXISTS post_egreso (
    id                  SERIAL PRIMARY KEY,
    paciente_id         INTEGER NOT NULL REFERENCES pacientes(id),
...
CREATE TABLE IF NOT EXISTS visitas (
    ...
    mod_diabetes        INTEGER DEFAULT 0,
    notas_clinicas      TEXT,
    fecha_registro      TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
-- ============================================================
CREATE TABLE IF NOT EXISTS farmacoterapia (
    id                  SERIAL PRIMARY KEY,
    visita_id           INTEGER NOT NULL REFERENCES visitas(id),
    paciente_id         INTEGER NOT NULL REFERENCES pacientes(id),
...
-- ============================================================
CREATE TABLE IF NOT EXISTS sibo_imo (
    id                  SERIAL PRIMARY KEY,
    visita_id           INTEGER NOT NULL REFERENCES visitas(id),
    paciente_id         INTEGER NOT NULL REFERENCES pacientes(id),
    fecha               DATE NOT NULL,
...
-- ============================================================
CREATE TABLE IF NOT EXISTS eii (
    id                  SERIAL PRIMARY KEY,
    visita_id           INTEGER NOT NULL REFERENCES visitas(id),
    paciente_id         INTEGER NOT NULL REFERENCES pacientes(id),
    fecha               DATE NOT NULL,
...
CREATE TABLE IF NOT EXISTS cohorte_pacientes (
...
CREATE TABLE IF NOT EXISTS farmacos_catalogo (
    ...
    egfr_g1 TEXT DEFAULT 'verde',
    egfr_g2 TEXT DEFAULT 'verde',
    egfr_g3a TEXT DEFAULT 'verde',
    egfr_g3b TEXT DEFAULT 'verde',
    egfr_g4 TEXT DEFAULT 'amarillo',
    egfr_g5 TEXT DEFAULT 'rojo',
    egfr_hd TEXT DEFAULT 'rojo',
    egfr_dp TEXT DEFAULT 'rojo',
    nota_egfr TEXT,
    precaucion_hepatica TEXT,
    interacciones TEXT,
...
CREATE TABLE IF NOT EXISTS interacciones_farmaco (
    ...
    clase_b TEXT,
    subclase_b TEXT,
    severidad TEXT NOT NULL CHECK(severidad IN ('contraindicado','grave','moderada','leve')),
...
CREATE TABLE IF NOT EXISTS farmacos_visita (
    id SERIAL PRIMARY KEY,
    visita_id INTEGER NOT NULL REFERENCES visitas(id),
    farmaco_id INTEGER NOT NULL REFERENCES farmacos_catalogo(id),
    dosis TEXT,
    frecuencia TEXT,
    via TEXT,
    semaforo_visita TEXT,
...
CREATE TABLE IF NOT EXISTS requerimientos_visita (
    id SERIAL PRIMARY KEY,
    visita_id INTEGER NOT NULL UNIQUE REFERENCES visitas(id),
    paciente_id INTEGER NOT NULL REFERENCES pacientes(id),
    fecha DATE NOT NULL,
    peso_actual REAL, talla_cm REAL,
    peso_ideal_hamwi REAL, peso_ideal_devine REAL, peso_ajustado REAL,
    peso_usado TEXT, peso_usado_kg REAL,
    formula_tmb TEXT, tmb_calculado REAL,
    factor_actividad REAL, factor_estres REAL DEFAULT 1.0,
    get_calculado REAL, kcal_objetivo REAL, deficit_kcal REAL, kcal_kg REAL,
    metodo_proteina TEXT, proteina_g_kg REAL, proteina_g_dia REAL, proteina_pct_kcal REAL,
    cho_g_dia REAL, cho_pct REAL, grasa_g_dia REAL, grasa_pct REAL, fibra_g_dia REAL,
    agua_ml_dia REAL, metodo_agua TEXT, restriccion_hidrica INTEGER DEFAULT 0,
    sodio_mg REAL, potasio_mg REAL, fosforo_mg REAL,
    restriccion_k INTEGER DEFAULT 0, restriccion_p INTEGER DEFAULT 0,
    meta_peso_kg REAL, meta_pct_perdida REAL, meta_cintura_cm REAL, meta_imc REAL,
    meta_hba1c REAL, meta_glucemia_ayunas REAL, meta_ldl REAL,
    meta_ta_sistolica INTEGER, meta_ta_diastolica INTEGER,
    meta_egfr REAL, meta_proteina_orina REAL,
    suplementacion TEXT, notas TEXT,
    fecha_registro TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE IF NOT EXISTS diagnostico_nutricional (
    id SERIAL PRIMARY KEY,
    visita_id INTEGER NOT NULL UNIQUE REFERENCES visitas(id),
    paciente_id INTEGER NOT NULL REFERENCES pacientes(id),
    fecha DATE NOT NULL,
    clasificacion_imc TEXT, riesgo_cintura TEXT, riesgo_icc TEXT,
    glim_perdida_peso_pct REAL,
    glim_perdida_peso_pos INTEGER DEFAULT 0, glim_imc_bajo_pos INTEGER DEFAULT 0,
    glim_masa_musc_red_pos INTEGER DEFAULT 0, glim_ingesta_red_pos INTEGER DEFAULT 0,
    glim_inflamacion_pos INTEGER DEFAULT 0, glim_causa_inflamacion TEXT,
    glim_diagnostico TEXT, glim_notas TEXT,
    sarc_fuerza_pos INTEGER DEFAULT 0, sarc_masa_pos INTEGER DEFAULT 0,
    sarc_rendimiento_pos INTEGER DEFAULT 0, sarc_iamm REAL,
    sarc_diagnostico TEXT, sarc_notas TEXT,
    rcm_glucosa TEXT, rcm_lipidos TEXT, rcm_ta TEXT,
    rcm_obesidad_central INTEGER DEFAULT 0, rcm_resultado TEXT,
    rcm_sindrome_metab INTEGER DEFAULT 0,
    estado_global TEXT, diagnostico_principal TEXT, plan_accion TEXT,
    fecha_registro TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE IF NOT EXISTS diabetes_visita (
    id SERIAL PRIMARY KEY,
    visita_id INTEGER NOT NULL UNIQUE REFERENCES visitas(id),
    paciente_id INTEGER NOT NULL REFERENCES pacientes(id),
    fecha DATE NOT NULL,
    glucemia_ayunas REAL, glucemia_postprandial REAL, glucemia_promedio REAL,
    hba1c REAL, hba1c_estimada REAL, fructosamina REAL, control_nivel TEXT,
    tar_en_rango REAL, tar_debajo_rango REAL, tar_encima_rango REAL, tar_hipoglucemia REAL,
    insulina_basal_tipo TEXT, insulina_basal_dosis REAL, insulina_basal_horario TEXT,
    insulina_rapida INTEGER DEFAULT 0, insulina_rapida_tipo TEXT,
    ratio_insulina_cho REAL, factor_sensibilidad REAL,
    gluco_ayunas_prom REAL, gluco_pre_almuerzo_prom REAL, gluco_post_almuerzo_prom REAL,
    gluco_pre_cena_prom REAL, gluco_post_cena_prom REAL, gluco_nocturna_prom REAL,
    hipo_episodios_semana INTEGER,
    hipo_nivel1 INTEGER DEFAULT 0, hipo_nivel2 INTEGER DEFAULT 0, hipo_nivel3 INTEGER DEFAULT 0,
    hipo_causa TEXT, hipo_horario TEXT,
    cho_total_g REAL, cho_desayuno_g REAL, cho_colacion1_g REAL,
    cho_almuerzo_g REAL, cho_colacion2_g REAL, cho_cena_g REAL,
    tratamiento_tipo TEXT, medicamentos_dm TEXT,
    meta_hba1c REAL, meta_glucemia_ayunas TEXT, meta_tar REAL, notas TEXT,
    fecha_registro TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE INDEX IF NOT EXISTS idx_req_visita  ON requerimientos_visita(visita_id);
CREATE INDEX IF NOT EXISTS idx_diag_visita ON diagnostico_nutricional(visita_id);
CREATE INDEX IF NOT EXISTS idx_dm_visita   ON diabetes_visita(visita_id);
